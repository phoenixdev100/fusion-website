  ...
  useEffect(() => {
    const fetchServerStats = async () => {
      try {
        ...
        const servers = {
          lobby: { address: 'standard.fusionnetwork.fun:40001', queryPort: 40001 },
          pvp: { address: 'standard.fusionnetwork.fun:40003', queryPort: 50012 },
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-black/30 p-4 sm:p-6 rounded-2xl border border-purple-500/20 hover:border-purple-500/50 transition-all duration-300">
                        ...
                        <div className="text-2xl sm:text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-400">
                          ...
                          {(lobbyStats.players.online + pvpStats.players.online + lifestealStats.players.online)+1}
                        </div>
                ...
                <Card className="relative overflow-hidden rounded-3xl backdrop-blur-sm bg-gray-900/50 border-0 hover:shadow-2xl hover:shadow-emerald-500/20 transition-all duration-300 group h-full">
                  ...
                  <CardHeader className="relative pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-medium ${lobbyStats.online ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                        {lobbyStats.online ? 'ONLINE' : 'OFFLINE'}
                      </div>
                    </div>
                  </CardHeader>
                        ...
                        <div className="bg-black/30 p-4 rounded-2xl border border-emerald-500/10 hover:border-emerald-500/30 transition-all duration-300">
                          ...
                          <div className="text-xl font-bold flex items-center gap-2">
                            ...
                            <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
                              {lobbyStats.players.online}/{(lobbyStats.players.online)+1}
                            </span>
                          </div>
                        </div>
                        <div className="bg-black/30 p-4 rounded-2xl border border-emerald-500/10 hover:border-emerald-500/30 transition-all duration-300">
                          ...
                          <div className="text-xl font-bold flex items-center gap-2">
                            ...
                            <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-teal-400">
                              {lobbyStats.performance.tps.toFixed(1)} TPS
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="w-full bg-black/30 rounded-full h-2.5 mb-1 overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${Math.min(lobbyStats.performance.tps / 20 * 100, 100)}%` }}
                          ></div>
                        </div>
                  ...
                  <CardHeader className="relative pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-medium ${lifestealStats.online ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                        {lifestealStats.online ? 'ONLINE' : 'MAINTENANCE'}
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    {lobbyStats.players.list.length > 0 && (
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                          {lobbyStats.players.list.slice(0, 11).map((player, index) => (
                            <motion.div
                          ...
                          {lobbyStats.players.online > 11 && (
                            ...
                            >
                              <div className="text-gray-400">
                                +{lobbyStats.players.online - Math.min(lobbyStats.players.list.length, 11)} more
                              </div>
                            </motion.div>
                          )}
                        </div>
